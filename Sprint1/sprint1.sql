...
#Ex4
SELECT COUNT(DISTINCT country) AS 'nº de paises realizando compras' FROM company
INNER JOIN transaction
ON company.id = transaction.company_id
WHERE declined = 0;
...
WHERE company.id = transaction.company_id AND declined = 0
...
FROM company, (SELECT * FROM transaction
				WHERE declined = 0 AND amount BETWEEN 100 AND 200) AS transaction100200
