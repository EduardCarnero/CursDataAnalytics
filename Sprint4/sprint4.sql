...
#Creación de JUNCTION TABLE entre transaction y product
...
SELECT product.id, transaction_product.product_id
FROM transaction_product
LEFT JOIN product ON product.id = transaction_product.product_id
...
#Ex1 CORRECIÓN
SELECT COUNT(product_id), product_id, product_name
FROM transaction_product, product
WHERE transaction_product.product_id = product.id
