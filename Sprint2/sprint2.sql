...
WHERE transaction.company_id = company.id AND country = (SELECT country FROM company WHERE company_name = 'Sed Id Limited');

#Ex2
SELECT company_name, amount
FROM company, transaction
WHERE transaction.company_id = company.id AND amount = (SELECT MAX(amount) FROM transaction);
